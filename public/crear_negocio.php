...
// Verificar que sea emprendedor (consulta corregida)
$stmt = $db->prepare("
    SELECT r.id_rol 
...
if (!$stmt->fetch()) {
    header('Location: perfil.php?error=No tienes el rol de emprendedor');
    exit;
}

$plantilla_id = $_GET['plantilla'] ?? 0;
if (!$plantilla_id) {
    header('Location: plantillas_disponibles.php');
    exit;
}

// Verificar que la plantilla existe
$stmt = $db->prepare("SELECT * FROM plantillas WHERE id_plantilla = ?");
    ...
    if (empty($nombre_comercial)) {
        ...
        try {
            ...
            $stmt = $db->prepare("
                ...
                VALUES (?, ?, ?, ?, 'Aprobado')
            ");
            $stmt->execute([$usuario['id'], $nombre_comercial, $nit, $descripcion]);
            $id_emprendimiento = $db->lastInsertId();
            
            // Crear personalización con la plantilla seleccionada
            $stmt = $db->prepare("
                INSERT INTO personalizacion_emprendimiento (id_emprendimiento, id_plantilla, color_primario, color_secundario)
                VALUES (?, ?, ?, ?)
            ");
            $stmt->execute([$id_emprendimiento, $plantilla_id, $plantilla['color_primario'], $plantilla['color_secundario']]);
            
            $db->commit();
            
            header('Location: dashboard_principal.php?success=1');
            exit;
        } catch (Exception $e) {
            $db->rollBack();
        ...
        <div class="card">
            ...
            <div class="plantilla-info">
                <div class="plantilla-colors">
                    <div class="plantilla-color" style="background: <?php echo $plantilla['color_primario']; ?>;"></div>
                    <div class="plantilla-color" style="background: <?php echo $plantilla['color_secundario']; ?>;"></div>
                </div>
                <div>
                    <div class="plantilla-name">Plantilla: <?php echo htmlspecialchars($plantilla['nombre']); ?></div>
            ...
            <div class="back-link">
                <a href="plantillas_disponibles.php">← Volver a plantillas</a>
            </div>
        </div>
    </div>
</body>
