...
if (count($roles_disponibles) <= 1) {
    // Si solo tiene un rol, redirigir directamente al dashboard principal
    $rol_unico = $roles_disponibles[0]['nombre_rol'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verificar que el rol esté entre los disponibles
    $rol_valido = false;
    foreach ($roles_disponibles as $rol) {
        if ($rol['nombre_rol'] === $rol_seleccionado) {
            $rol_valido = true;
            break;
        }
    }
    ...
    if ($rol_valido) {
        $_SESSION['rol_activo'] = $rol_seleccionado;
        header('Location: dashboard_principal.php');
        exit;
    } else {
        $error = 'Rol no válido';
        ...
        <form method="POST" id="rolForm">
            <div class="roles-grid">
                ...
                <label class="rol-option">
                    <input type="radio" name="rol" value="<?php echo $rol['nombre_rol']; ?>">
                    <div class="rol-icon">
