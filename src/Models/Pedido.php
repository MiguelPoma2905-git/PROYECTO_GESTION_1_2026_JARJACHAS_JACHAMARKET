...
class PedidoModel {
    ...
    public function crearPedido($datos) {
        ...
        try {
            ...
            $stmt = $this->conn->prepare("
                INSERT INTO envios_logistica (id_pedido) VALUES (?)
            ");
    ...
    public function getPedidosPendientesRepartidor() {
        $stmt = $this->conn->prepare("
    ...
    public function asignarRepartidor($id_pedido, $id_repartidor) {
        $stmt = $this->conn->prepare("
            ...
            WHERE id_pedido = ?
        ");
        if ($stmt->execute([$id_repartidor, $id_pedido])) {
            $stmt = $this->conn->prepare("
    ...
    public function marcarEntregado($id_pedido, $id_repartidor) {
        $stmt = $this->conn->prepare("
            UPDATE envios_logistica 
            SET fecha_entrega = NOW()
            WHERE id_pedido = ? AND id_repartidor = ?
        ");
        if ($stmt->execute([$id_pedido, $id_repartidor])) {
            $stmt = $this->conn->prepare("
