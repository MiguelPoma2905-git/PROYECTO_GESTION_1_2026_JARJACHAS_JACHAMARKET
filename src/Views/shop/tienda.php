...
<head>
    ...
    <title><?= htmlspecialchars($emprendimiento['nombre_comercial']) ?> - Jacha Marketplace</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            background: <?= $emprendimiento['color_fondo'] ?? '#0a0a0a' ?>;
            color: #e8e8e8;
            line-height: 1.5;
        }
        .header {
            background: <?= $emprendimiento['color_primario'] ?? '#1a1a1a' ?>;
            padding: 20px 32px;
            display: flex;
        ...
        .producto-card:hover { transform: translateY(-4px); border-color: <?= $emprendimiento['color_primario'] ?? '#fff' ?>; }
        ...
        .producto-precio { font-size: 22px; font-weight: 600; color: <?= $emprendimiento['color_primario'] ?? '#fff' ?>; margin: 12px 0; }
        .btn-comprar {
            display: block; width: 100%; padding: 10px; background: <?= $emprendimiento['color_secundario'] ?? '#333' ?>;
...
<body>
    ...
    <div class="container">
        <h1 class="shop-title"><?= htmlspecialchars($emprendimiento['nombre_comercial']) ?></h1>
        <p class="shop-desc"><?= htmlspecialchars($emprendimiento['descripcion']) ?></p>
        ...
        <div class="productos-grid">
            ...
            <div class="producto-card">
                <img src="<?= BASE_URL ?>/<?= $producto['imagen_url'] ?: 'assets/images/placeholder.png' ?>" class="producto-img" onerror="this.src='<?= BASE_URL ?>/assets/images/placeholder.png'">
                <div class="producto-info">
                    <h3><?= htmlspecialchars($producto['nombre']) ?></h3>
                    <?php if ($atributos && isset($atributos['marca'])): ?>
                        <small style="color:#888;"><?= htmlspecialchars($atributos['marca']) ?></small>
                    <?php endif; ?>
                    <div class="producto-precio">Bs. <?= number_format($producto['precio_base'], 2) ?></div>
