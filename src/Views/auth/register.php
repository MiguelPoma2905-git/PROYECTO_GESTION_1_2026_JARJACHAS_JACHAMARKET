...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body::before { content: ''; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-image: url('<?= BASE_URL ?>/assets/images/fondo_1.jpg'); background-size: cover; background-position: center; opacity: 0.12; pointer-events: none; }
        ...
        .register-container { position: relative; z-index: 1; width: 100%; max-width: 500px; margin: 40px 24px; background: rgba(8,8,8,0.65); backdrop-filter: blur(15px); border-radius: 32px; border: 1px solid rgba(255,255,255,0.1); padding: 48px 40px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); animation: fadeInUp 0.6s ease-out both; }
...
<body>
    ...
    <div class="register-container">
        ...
        <form method="POST" action="<?= BASE_URL ?>/registro" id="registroForm">
            ...
            <button type="submit" class="btn-register" id="submitBtn">Continuar</button>
        </form>
    ...
    <script>
        ...
        function validateMatch() { const match = password.value === confirmPassword.value && password.value.length > 0; requirements.match = match; updateRequirementUI('match', match); }
